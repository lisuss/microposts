...
function addEditPost() {
    ...
    if (title === '' || body === '') {
        _ui.ui.showAlert('Please fill fields', 'alert alert-danger');
    } else {
        var _data = {
            title: title,
            body: body
        ...
        if (id === '') {
            ...
            // Update post
            _http.http.put('http://localhost:3000/posts/' + id, _data).then(function (data) {
                _ui.ui.showAlert('Post updated', 'alert alert-success');
                _ui.ui.changeState('add');
                getPosts();
            }).catch(function (err) {
                return console.log(err);
            });
...
function editState(e) {
    if (e.target.parentElement.classList.contains('edit')) {
        var id = e.target.parentElement.dataset.id;
        var _title = e.target.parentElement.previousElementSibling.previousElementSibling.textContent;
        var _body = e.target.parentElement.previousElementSibling.textContent;
